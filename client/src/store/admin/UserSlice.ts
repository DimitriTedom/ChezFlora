...
>(
    ...
    async (params: FetchUsersParams, { rejectWithValue }) => {
        try {
            const response = await axios.get(`${API_URL}admin/users/get`, { params:{
                page: params.page,
                limit: params.limit,
                role: params.role,
...
>(
    ...
    async (payload: UpdateUserRolesPayload, { rejectWithValue }) => {
        try {
            const response = await axios.patch(`${API_URL}admin/users/update-roles`, payload, { withCredentials: true });
...
>(
    ...
    async (payload: DeleteUsersPayload, { rejectWithValue }) => {
        try {
            const response = await axios.delete(`${API_URL}admin/users/delete`, { data: payload, withCredentials: true });
            return response.data.message;
        } catch (error: any) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(updateUserRoles.rejected, (state, action) => {
                state.isUpdating = false;
                state.error = action.payload as string || "Failed to update user roles";
            })
            ...
            .addCase(deleteUsers.fulfilled, (state, action) => {
                state.isUpdating = false;
                state.users = state.users.filter((user:User) => !action.meta.arg.userIds.includes(user.id));
                state.pagination.total = state.pagination.total - action.meta.arg.userIds.length;
