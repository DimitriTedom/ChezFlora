...
import { checkPendingUser, completeRegistration, } from "@/store/authSlice"; // action à dispatch pour renvoyer l'OTP
import { useCustomToast } from "@/hooks/useCustomToast";

const CompleteRegistrationOTP: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await dispatch(completeRegistration({ email: email!, otp })).unwrap().then((data) => {
        if (data?.success) {
          showToast({
  ...
  const handleResendOtp = async () => {
    ...
    setLoading(true);
    try {
      dispatch(checkPendingUser(email))
        .unwrap()
        .then((data) => {
          showToast({
